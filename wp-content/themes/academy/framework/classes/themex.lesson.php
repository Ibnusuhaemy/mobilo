		...
		if(self::$data['progress']==0) {
			...
			if(!empty(self::$data['quiz']['questions'])) {
				...
				if(self::$data['quiz']['percentage']>0 && $percentage>=self::$data['quiz']['percentage']) {
					ThemexInterface::$messages[]=sprintf(__('Selamat! Anda telah menuntaskan latihan 100% %!', 'academy'), $percentage);
					self::completeLesson($percentage, true);
				} else {
					ThemexInterface::$messages[]=sprintf(__('Anda harus menuntaskan latihan dengan 100% % jawaban benar untuk melanjutkan.', 'academy'), self::$data['quiz']['percentage']);
				}
			}
		} else {
