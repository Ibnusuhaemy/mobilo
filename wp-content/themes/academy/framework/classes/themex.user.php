...
class ThemexUser {
	...
	public static function registerUser($data) {
		...
		if(empty($data['user_email']) || empty($data['user_login']) || empty($data['user_password']) || empty($data['user_password_repeat'])) {
			ThemexInterface::$messages[]=__('Mohon isi semua field', 'academy');
		} else {
			if(!is_email($data['user_email'])) {
				ThemexInterface::$messages[]=__('Alamat email tidak valid', 'academy');
			} else if(email_exists($data['user_email'])) {
				ThemexInterface::$messages[]=__('Alamat email sudah digunakan', 'academy');
			}
			...
			if(strlen($data['user_password'])<4) {
				ThemexInterface::$messages[]=__('Panjang password minimal 4 karakter', 'academy');
			} else if(strlen($data['user_password'])>16) {
				ThemexInterface::$messages[]=__('Password tidak boleh melebihi 16 karakter', 'academy');
			} else if(preg_match("/^([a-zA-Z0-9]{1,20})$/", $data['user_password'])==0) {
				ThemexInterface::$messages[]=__('Password mengandung karakter invalid', 'academy');
			} else if($data['user_password']!=$data['user_password_repeat']) {
				ThemexInterface::$messages[]=__('Passwords tidak sama', 'academy');
			}
		}
	...
	public static function loginUser($data) {
		...
		if(is_wp_error($user) || empty($data['user_login']) || empty($data['user_password'])){
			ThemexInterface::$messages[]=__('Username atau password salah', 'academy');
		} else {
